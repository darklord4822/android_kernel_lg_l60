...
extern struct  alsps_hw *cm36671_get_cust_alsps_hw(void);
static int cm_local_init(void);
static int cm36671_remove(void);
static struct sensor_init_info cm36671_init_info =
{
    .name  = "cm36671",
    .init     = cm_local_init,
    .uninit   = cm36671_remove,
};

static int cm36671_init_flag = -1;
/*----------------------------------------------------------------------------*/
...
static int cm36671_get_ps_value(struct cm36671_priv *obj, u8 ps)
{
	...
	//APS_LOG("cm36671_get_ps_value = %d, ps = %d;thd_high=%d,thd_low=%d \n",val,ps,obj->ps_thd_val_high, obj->ps_thd_val_low);
...
void cm36671_psensor_lock(int enable_pflag){

	APS_ERR("[%s], enable_pflag=%d\n",__func__,enable_pflag);
...
static int cm36671_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	obj->hw = cm36671_get_cust_alsps_hw();//get custom file data struct
	...
	if( err = cm36671_create_attr(&(cm36671_init_info.platform_diver_addr->driver)) )
	{
...
{
	//APS_FUN();  
	struct alsps_hw *hw = cm36671_get_cust_alsps_hw();
...
static int __init cm36671_init(void)
{
	...
	if(hwmsen_alsps_sensor_add(&cm36671_init_info))
	{
