...
int bq24158_read_byte(kal_uint8 cmd, kal_uint8 *returnData)
{
    ...
    {    
        ...
        battery_xlog_printk(BAT_LOG_FULL, "~=>R:0x%X >Fail\n", cmd);
        return 0;
    }
    ...
    *returnData = readData;
    ...
    mutex_unlock(&bq24158_i2c_access);
    battery_xlog_printk(BAT_LOG_FULL, "~=>R:0x%X: 0x%X >OK\n", cmd, readData);
    return 1;
}
...
int bq24158_write_byte(kal_uint8 cmd, kal_uint8 writeData)
{
    ...
    if (ret < 0) 
    {
        ...
        new_client->ext_flag=0;
        mutex_unlock(&bq24158_i2c_access);
        battery_xlog_printk(BAT_LOG_CRTI, "~=>Write:0x%X,0x%X > Fail\n", cmd,writeData);
        return 0;
    }
    
    new_client->ext_flag=0;
    mutex_unlock(&bq24158_i2c_access);
    battery_xlog_printk(BAT_LOG_CRTI, "~=>Write:0x%X,0x%X > OK\n", cmd,writeData);
    return 1;
}
